...
function fallbackSchedule(input) {
  ...
  return { title: input.slice(0, 80), time: String(hour).padStart(2, '0') + ':' + minute, kind: /gmail|email|mail/i.test(input) ? 'gmail' : 'vscode', filePath: 'scheduled-note.txt', content: input, command: (input.match(/npm\s+(run\s+)?(dev|start|test|build)/i)?.[0] || '') };
}
export async function createServer() {
  ...
  api.post('/api/scheduled/interpret', async (req, res) => { const input=String(req.body?.instruction||'').trim(); if(input.length<3)return res.status(400).json({error:'Describe the scheduled task first.'}); try { if(!process.env.OPENAI_API_KEY){ const match=input.match(/(?:at|@)[ ]*(\d{1,2})(?::(\d{2}))?[ ]*(am|pm)?/i); let hour=match?Number(match[1]):9; const minute=match?.[2]||'00'; const suffix=(match?.[3]||'').toLowerCase(); if(suffix==='pm'&&hour<12)hour+=12; if(suffix==='am'&&hour===12)hour=0; return res.json({title:input.slice(0,80),time:String(hour).padStart(2,'0')+':'+minute,kind:/gmail|email|mail/i.test(input)?'gmail':'vscode',filePath:'scheduled-note.txt',content:input, command:(input.match(/npm\s+(run\s+)?(dev|start|test|build)/i)?.[0] || '')}); } const client=new OpenAI({apiKey:process.env.OPENAI_API_KEY}); const result=await client.chat.completions.create({model:process.env.OPENAI_MODEL||'gpt-5-mini',messages:[{role:'user',content:'Convert this scheduled desktop task into JSON. If the instruction asks for code, generate complete runnable code in content. Instruction: '+input+' Return only title, time HH:MM, kind vscode or gmail, filePath, content, and optional command. command may only be npm run dev, npm start, npm test, npm build, node file.js, or python file.py. Use daily scheduling. Never include passwords or claim an email was sent.'}],response_format:{type:'json_object'},max_completion_tokens:400}); const raw=result.choices[0]?.message?.content||'{}'; const parsed = JSON.parse(raw); return res.json(parsed.time && parsed.kind ? parsed : fallbackSchedule(input)); } catch(error){ console.error('[scheduled interpret]',error.message); return res.status(400).json({error:'AI could not understand this schedule.'}); } });
